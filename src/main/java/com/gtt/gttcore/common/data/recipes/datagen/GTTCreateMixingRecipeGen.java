        ...
        for (GTRecipeBuilder recipeBuilder : GTTRecipeTypes.toReRegisterCreateMixing) {
            create(GTTCore.id("gregtech_" + recipeBuilder.id.getPath()), b -> {
                if (recipeBuilder.input.containsKey(ItemRecipeCapability.CAP))
                    recipeBuilder.input.get(ItemRecipeCapability.CAP)
                            .stream()
                            .filter(content -> content.chance != 0)
                            .forEach(content -> b.require((Ingredient) content.content));
                if (recipeBuilder.input.containsKey(FluidRecipeCapability.CAP))
                    recipeBuilder.input.get(FluidRecipeCapability.CAP)
                            .forEach(content ->
                                    Arrays.stream(((FluidIngredient) content.content).getStacks())
